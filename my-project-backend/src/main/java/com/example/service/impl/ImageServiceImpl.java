...
public class ImageServiceImpl extends ServiceImpl<ImageStoreMapper, StoreImage> implements ImageService {
    ...
    @Override
    public String uploadImageCache(MultipartFile file, int id) throws IOException {
        //需要添加上传记录，用于判断是谁上传的图片
        //TODO:后续可以在此基础上加上上传图片大小等更加详细的记录
        String key= Const.FORUM_IMAGE_COUNTER+id;
        //TODO：对用户上传进行限制（1小时20张图片？）---后期看看可以使用图床功能？
